...
MODULE PARAM
    ...
    INTEGER,PARAMETER :: NX=200 !Number of Cells in X-direction (nodes number 0 to nx)
    INTEGER,PARAMETER :: NY=800 !Number of Cells in Y-direction (nodes number 0 to ny)
	...
	DOUBLE PRECISION,PARAMETER :: DIAM_PHYSICAL=0.2 !cm
	DOUBLE PRECISION,PARAMETER :: L_C=20.0	!Lattice Units Characteristic Length of the problem
	...
	INTEGER,PARAMETER :: TIME_MAX=NINT(8.0/C_T) !NINT(5.01*PERIOD_OSC)
	INTEGER,PARAMETER :: TIME_SAMPLE=NINT(0.25/C_T)
...
PROGRAM MAIN
	...
	DOUBLE PRECISION :: F_H_IN(1:3,1:2),VP_OLD(1:3,1:2)
	TYPE(PARTICLE) :: DISK(1:2) !???????
	!********************************************************************************
	...
	!WRITE(11,*)'VARIABLES=T,Y,X,ANG,Ux,Uy,OMG,Fx,Fy,Tr,U_W'
	...
	!END DO
	!STOP
	DISK(1)%CENTER=(/1.0/C_L,7.2/C_L/) !initial particle center position
	DISK(2)%CENTER=(/0.97/C_L,6.8/C_L/)
	DISK%ANGLE=0D0
	...
	!PRINT *,KK
		...
		PRINT *,KK,DISK(1)%CENTER(2),DISK(2)%CENTER(2)
	...
	DO J=0,NY
		DO I=0,NX
			...
			IF( DISK(IP)%PHI(I,J)>=0.5 ) THEN
				...
				D_P=20.D0-( (DBLE(I)-XPR)**2+(DBLE(J)-YPR)**2 )
				IF(D_P>=0)THEN
					DOMAIN_IMAGE(I,J)=15
				END IF
...
SUBROUTINE ComputeCollisionForce(DISK)  
	...
	!EPS_P=1.0E6;EPS_W=0.5*EPS_P;E_P=2.0E6;E_W=0.5*E_P
	!EPS_P=0.5;EPS_W=EPS_P;E_P=10.0;E_W=E_P
	EPS_P=1.0;EPS_W=EPS_P;E_P=20.0;E_W=E_P
