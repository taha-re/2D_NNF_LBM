...
MODULE PARAM
    ...
    INTEGER,PARAMETER :: NX=240 !Number of Cells in X-direction (nodes number 0 to nx)
    INTEGER,PARAMETER :: NY=720 !Number of Cells in Y-direction (nodes number 0 to ny)
	...
	DOUBLE PRECISION,PARAMETER :: RHO_S=1.25
	DOUBLE PRECISION,PARAMETER :: RHO_F=1.0 !Lattice Units
	DOUBLE PRECISION,PARAMETER :: NU=0.05	!Lattice Units (Kinematic Viscosity of La
	DOUBLE PRECISION,PARAMETER :: NU_PHYSICAL=0.1 !cm^2/sec
	DOUBLE PRECISION,PARAMETER :: DIAM_PHYSICAL=0.25 !cm
	DOUBLE PRECISION,PARAMETER :: L_C=30.0	!Lattice Units Characteristic Length of the problem
	!DOUBLE PRECISION,PARAMETER :: FREQ_PHYSICAL=1.0 !Hz
	...
	INTEGER,PARAMETER :: TIME_MAX=INT(1.0/C_T)
	
	!DOUBLE PRECISION,PARAMETER :: A_C=L_C !Amplitude in LU
	!DOUBLE PRECISION,PARAMETER :: FREQ_OSC=FREQ_PHYSICAL*C_T !Angular Frequency of Oscillation=Max Vel LB/(2*pi*amplitude)
	!DOUBLE PRECISION,PARAMETER :: PERIOD_OSC=1.0/FREQ_OSC
	!DOUBLE PRECISION,PARAMETER :: U_C=1.
	!INTEGER,PARAMETER :: TIME_SAMPLE=INT(1.0*FREQ_OSC/C_T) ! 1 SHOULD BE CHANGED FOR EACH CASE
	DOUBLE PRECISION,PARAMETER :: T_C=1.0/C_T
	DOUBLE PRECISION,PARAMETER :: F_C=Nu*U_C
	DOUBLE PRECISION,PARAMETER :: A_G=-9.81/C_G !Lattice Units
	DOUBLE PRECISION,PARAMETER :: Tau_y=BN_T*NU*U_C/L_C !Y_G*(RHO_S-RHO_F)*(-A_G)*L_C !yield stress BN_T*NU*U_T/L_C
	...
	DOUBLE PRECISION,PARAMETER :: M_PAP=1.0E8 !FOR PAPANASTASIOU MODEL 
...
PROGRAM MAIN
    ...
    DOUBLE PRECISION :: AvgDen,SSUM2,ER,START,FINISH,MA,INIT_WALL_CENTER(1:2),PHI_T(0:NX,0:NY),RE_P,E_T,E_R
    DOUBLE PRECISION :: RHO_0_RHO_P,INV_MP,INV_IP
	INTEGER :: KK,I,J,IP,IW,Domain_Image(0:NX,0:NY)
	DOUBLE PRECISION :: F_H_IN(1:3,1:1),VP_OLD(1:3,1:1)
	TYPE(PARTICLE) :: DISK(1:1) !???????
	...
	WRITE(12,*) 'TAU_LB=',1.0/OMEGA;WRITE(12,*) 'U_LB_max=',U_C;WRITE(12,*) 'Tau_Y_LB=',TAU_Y; !2.0*PI*A_C*FREQ_OSC;WRITE(12,*) 'FREQ_LB=',FREQ_OSC;
	!WRITE(12,*) 'MAX_ITR (per cycle)=',1.0/FREQ_OSC;WRITE(12,*) '**********************';WRITE(12,*) 'Physical Parameter'
	...
	!WRITE(11,*)'VARIABLES=T,Y_P_REL,X_P_REL,Ux,Uy_REL,Omg,Fx,Fy,Tr,E_T,E_R,Y_W_E1,UY_W'
	OPEN(10,FILE='02_Domain_Image.plt')
	WRITE(10,*)'VARIABLES =X, Y, Image'
	!OPEN(1001,FILE='00_WallParticle_Sample.TXT')
	!WRITE(1001,*)'VARIABLES =kk,v_y_wall_lb,y_wall,v_y_p,v_x_p,Y_p,X_p'
    CALL CPU_TIME(START)
	...
	Domain_Image=0
	!***WALL PARTICLES*****************************************************************************************
	WALL%HEIGHT=2.0*DBLE(NY)
	WALL%WIDTH=2.0*DBLE(NX)
	WALL%THICKNESS=10.0
	WALL%CENTER=(/0.5*DBLE(NX),0.5*DBLE(NY)/); !3.0*A_C+WALL%THICKNESS+0.5*WALL%HEIGHT
	...
	WALL%VELOCITY=(/0D0,0D0,0D0/) !initial velocity of particle 2.0*PI*A_C*COS(2.0*PI*FREQ_OSC*0.0)
	WALL%FORCE=(/0.0,0.0,0.0/) !initial forces on particle
	WALL%ZITA=1.0
	!***'Particles"**************************************************************
	OPEN(200,FILE='0_Particle_Initial_Position.txt');
	DO IP=1,N_PARTICLE
	READ(200,*) DISK(IP)%CENTER,DISK(IP)%DIAM,DISK(IP)%RHO_P
	...
	!DO IP=1,N_PARTICLE
	!WRITE(*,*) DISK(IP)%CENTER,DISK(IP)%DIAM
	!END DO
	!STOP
	...
	!PRINT *,KK
		...
		E_T=0.5*(RHO_S*PI*(0.5*DIAM_PHYSICAL)**2)*(DISK(1)%VELOCITY(1)*DISK(1)%VELOCITY(1)+DISK(1)%VELOCITY(2)*DISK(1)%VELOCITY(2))*(C_U**2)
		WRITE(11,*) KK*C_T,DISK(1)%CENTER(2)*C_L,DISK(1)%VELOCITY(2)*C_U,RE_P,E_T
		PRINT *,KK,DISK(1)%CENTER(2)*C_L

		!WRITE(11,*) KK*C_T,DISK(1)%CENTER(2)*C_L,DISK(2)%CENTER(2)*C_L,&
		!			&(DISK(1)%CENTER(1)-0.5*DBLE(NX))*C_L,(DISK(2)%CENTER(1)-0.5*DBLE(NX))*C_L,&
		!			&DISK(1)%VELOCITY(1)*C_U,DISK(2)%VELOCITY(1)*C_U,&
		!			&DISK(1)%VELOCITY(2)*C_U,DISK(2)%VELOCITY(2)*C_U,&
		!			&DISK(1)%VELOCITY(3)*T_C,DISK(2)%VELOCITY(3)*T_C,&
		!			&(DISK(1)%FORCE(1)+F_H_IN(1,1)),(DISK(2)%FORCE(1)+F_H_IN(1,2)),&
		!			&(DISK(1)%FORCE(2)+F_H_IN(2,1)),(DISK(2)%FORCE(2)+F_H_IN(2,2)),&
		!			&(DISK(1)%FORCE(3)+F_H_IN(3,1)),(DISK(2)%FORCE(3)+F_H_IN(3,2)),&
		!			&(SQRT((DISK(1)%CENTER(1)-DISK(2)%CENTER(1))**2+(DISK(1)%CENTER(2)-DISK(2)%CENTER(2))**2)-L_C)*C_L,&
		!			& (DISK(1)%CENTER(1)-DISK(2)%CENTER(1))*C_L,&
		!			& (DISK(1)%CENTER(2)-DISK(2)%CENTER(2))*C_L
		!PRINT *,KK,DISK(1)%CENTER(2)*C_L,DISK(2)%CENTER(2)*C_L

		!***Wall Dynamics Calculation****************
		!WALL%VELOCITY(2)=2.0*PI*A_C*FREQ_OSC*SIN(2.0*PI*FREQ_OSC*KK+0.5*PI)
		!IF(KK<INT(0.25*PERIOD_OSC))THEN
		!WALL%VELOCITY(2)=(4.0*FREQ_OSC)*KK*2.0*PI*A_C*FREQ_OSC*SIN(2.0*PI*FREQ_OSC*KK+0.5*PI)
		!END IF
		...
		!IF(MOD(KK,1000)==0)THEN	
				...
				!END DO
				!END DO		
				!CALL WriteResults(UX,UY,RHO,DOMAIN_IMAGE,STRESS,WALL,kk)
				!WRITE(1001,*) KK,WALL%VELOCITY(2)
		!END IF
	...
	END DO
	END DO
    call WriteResults(UX,UY,RHO,DOMAIN_IMAGE,STRESS,WALL,KK);!WRITE(1001,*) KK,WALL%VELOCITY(2)
...
SUBROUTINE ComputeConcenterationFunction(DISK,WALL,DOMAIN_IMAGE,IS_SOLID_NODE,IS_PARTICLE_NODE,IS_WALL_NODE)
	...
	DOMAIN_IMAGE=5;IS_SOLID_NODE=.FALSE.;IS_WALL_NODE=.TRUE.;IS_PARTICLE_NODE=.FALSE.
	...
	DO J=0,NY
		DO I=0,NX
			...
			ELSEIF(DBLE(I)<=EDGE_LEFT .OR. DBLE(I)>=EDGE_RIGHT .OR. DBLE(J)<=EDGE_BOTTOM .OR. DBLE(J)>=EDGE_TOP)THEN
				...
				END IF
				IS_WALL_NODE(I,J)=.FALSE.
				IS_SOLID_NODE(I,J)=.TRUE. !OUT OF WALL PARTICLE
				DOMAIN_IMAGE(I,J)=-1
			END IF
		ENDDO
	ENDDO
...
SUBROUTINE ComputeCollisionForce(DISK,WALL)  
	...
	ZITA=2.0;CIJ=1.0;EPS_P=1.0;EPS_W=2.0*EPS_P;
	!DO IP=1,N_PARTICLE
	...
	!*******************GLOWINSKY MODEL**************
	IF(1)THEN
	ZITA=2.0;CIJ=-(RHO_S-RHO_F)*A_G*PI*0.5*DISK(1)%DIAM*0.5*DISK(1)%DIAM !A_G*PI*0.5*DISK(1)%DIAM*0.5*DISK(1)%DIAM;
	EPS_P=1.0E5;EPS_W=0.5*EPS_P;E_P=2.0E5;E_W=0.5*E_P
	...
	DO IP=1,N_PARTICLE
		DO IW=1,4 !Index of each Wall
			...
			R_W(1)=2.0*DBLE(NX)-DISK(IP)%CENTER(1);R_W(2)=DISK(IP)%CENTER(2)  !2.0*DBLE(NX)-DISK(IP)%CENTER(1)
...
SUBROUTINE Collision_MRT(UX,UY,RHO,F,FEQ,G,Omega_BN,Domain_Image,STRESS)
    ...
    DO J=0,nY
        DO I=0,nX
			...
			Inv_Rho=1.0/RHO(I,J); ZARIB=-1.5*d_mrt*Inv_Rho
			D11=ZARIB*(F_1(1)+F_1(5)+F_1(6)+F_1(3)+F_1(7)+F_1(8)) !rate of deformation tensor Components d_ij
			D22=ZARIB*(F_1(5)+F_1(2)+F_1(6)+F_1(7)+F_1(4)+F_1(8))
			D12=ZARIB*(F_1(5)-F_1(6)+F_1(7)-F_1(8))
			ZARIB=0.75*Inv_Rho
			D11=D11+ZARIB*( (OMEGA_MRT(7)-OMEGA_MRT(1))*(G(1,I,J)*UX(I,J)+G(2,I,J)*UY(I,J))-OMEGA_MRT(7)*(2.0*G(1,I,J)*UX(I,J)) )
			D22=D22+ZARIB*( (OMEGA_MRT(7)-OMEGA_MRT(1))*(G(1,I,J)*UX(I,J)+G(2,I,J)*UY(I,J))-OMEGA_MRT(7)*(2.0*G(2,I,J)*UY(I,J)) )
			D12=D12+ZARIB*( -OMEGA_MRT(7)*(G(1,I,J)*UY(I,J)+G(2,I,J)*UX(I,J)) )
			!**************************
			GAMMA=SQRT(2.0*ABS(D11**2+D22**2+2.0*D12**2)); !Shear Rate (Original Definition)
			!GAMMA=SQRT(4.0*ABS(D12**2-D11*D22)); !Shear Rate (Only for Incompressible Flow)
			!***Power-Law Model**************************************
...
!===================================================================
SUBROUTINE WriteResults(UX,UY,RHO,DOMAIN_IMAGE,STRESS,WALL,TSTEP)
    USE PARAM
    IMPLICIT NONE
...
SUBROUTINE WRITE_DOMAINIMAGE(DOMAIN_IMAGE,TSTEP,FILE_NUMBER)
    ...
    DO J=0,NY
       WRITE(FILE_NUMBER,*)(DBLE(I-NX/2),I=0,NX)
    END DO
    DO J=0,NY
