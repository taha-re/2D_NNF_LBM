!By :: Taha Rezaee (reazaee.taha@gmail.com)
MODULE PARAM
	DOUBLE PRECISION,PARAMETER :: PI=3.14159265359
	LOGICAL,PARAMETER :: NON_NEWTONIAN= .TRUE.
	LOGICAL,PARAMETER :: READ_INPUT= .FALSE.
	LOGICAL,PARAMETER :: WRITE_OUTPUT= .FALSE.

    INTEGER,PARAMETER :: NX=80 !Number of Cells in X-direction (nodes number 0 to nx)
    INTEGER,PARAMETER :: NY=300 !Number of Cells in Y-direction (nodes number 0 to ny)
	...
	DOUBLE PRECISION,PARAMETER :: Y_g=BN_S	!Lattice Units (Yeild Number or Bingham Number
	DOUBLE PRECISION,PARAMETER :: RHO_S=2.0
	...
	DOUBLE PRECISION,PARAMETER :: L_C=DBLE(NX/4)	!Lattice Units Characteristic Length of the problem
	DOUBLE PRECISION,PARAMETER :: Omg_PHYSICAL=15.0 !rad/s
	DOUBLE PRECISION,PARAMETER :: A_G=-RE_S*NU*NU/((L_C**3)*(RHO_S-RHO_F)*RHO_F)
	DOUBLE PRECISION,PARAMETER :: U_C=(L_C**2)*(RHO_S-RHO_F)*(-A_G)/NU   ! Characteristic Velocity of Problem
	...
	DOUBLE PRECISION,PARAMETER :: A_C=0d0*L_C !Amplitude in LU
	...
	INTEGER,PARAMETER :: TIME_MAX=NINT(1.6*L_C*L_C/NU)
	INTEGER,PARAMETER :: TIME_SAMPLE=NINT(0.25*PERIOD_OSC)
	...
	DOUBLE PRECISION,PARAMETER :: M_PAP=5.5E6 !FOR PAPANASTASIOU MODEL 
...
PROGRAM MAIN
	...
	DOUBLE PRECISION :: F_H_IN(1:3,1),VP_OLD(1:3,1)
	...
	WRITE(12,*) 'TAU_LB=',1.0/OMEGA;WRITE(12,*) 'U_LB_max=',U_C;WRITE(12,*) 'Tau_Y_LB=',TAU_Y;WRITE(12,*) 'A_G=',A_G;
	...
	WRITE(11,*)'VARIABLES=T,Y,X,ANG,Ux,Uy,OMG,Fx,BnT,Cd,ReT'
	!OPEN(12,FILE='01_ParticleTimehistory_NonDimensional.plt')
	...
	!END DO
	!STOP
	DISK(1)%CENTER=(/0.5*DBLE(NX),DBLE(NY)-7.0*L_C/) !initial particle center position
	DISK(1)%ANGLE=0D0
	...
	!PRINT *,KK
		...
		WRITE(11,*) KK*NU/L_C**2,DISK(1)%CENTER(2),(DISK(1)%CENTER(1)),DISK(1)%ANGLE,&
					&DISK(1)%VELOCITY(1),DISK(1)%VELOCITY(2)/U_C,DISK(1)%VELOCITY(3),&
					&(DISK(1)%FORCE(1)+F_H_IN(1,1)),TAU_Y*L_C/(NU*-DISK(1)%VELOCITY(2)),(DISK(1)%FORCE(2)+F_H_IN(2,1))/(NU*-DISK(1)%VELOCITY(2)),&
					&-DISK(1)%VELOCITY(2)*L_C/NU
		PRINT *,KK,TAU_Y*L_C/(NU*-DISK(1)%VELOCITY(2)),-DISK(1)%VELOCITY(2)*L_C/NU
		...
		IF( KK>=NINT(7.0*PERIOD_OSC) .AND. MOD(KK,TIME_SAMPLE)==0)THEN	
...
SUBROUTINE BOUNDARYCONDITION(F,F_POST,RHO,UX,UY,KK)
	...
	UXN=0.0;UYN=0D0 !A_C*OMG_OSC*SIN(OMG_OSC*DBLE(KK))
	UXS=0.0;UYS=0D0 !A_C*OMG_OSC*SIN(OMG_OSC*DBLE(KK))
	DO I=1,NX-1
	...
	UX_L=0.0;UY_L=0D0 !A_C*OMG_OSC*SIN(OMG_OSC*DBLE(KK));
	Ux_R=0.0;Uy_R=0D0 !A_C*OMG_OSC*SIN(OMG_OSC*DBLE(KK));
    DO J=1,NY-1
