...
MODULE PARAM
    ...
    INTEGER,PARAMETER :: NX=1500 !Number of Cells in X-direction (nodes number 0 to nx)
    INTEGER,PARAMETER :: NY=1000 !Number of Cells in Y-direction (nodes number 0 to ny)
	...
	DOUBLE PRECISION,PARAMETER :: RE_T=100.0
	!DOUBLE PRECISION,PARAMETER :: BN_T=2.73
	DOUBLE PRECISION,PARAMETER :: Y_g=0.0	!Lattice Units (Yeild Number or Bingham Number
	DOUBLE PRECISION,PARAMETER :: RHO_S=1.0	!Lattice Units
	DOUBLE PRECISION,PARAMETER :: RHO_F=1.0 !Lattice Units
	DOUBLE PRECISION,PARAMETER :: NU=0.025	!Lattice Units (Kinematic Viscosity of La
	DOUBLE PRECISION,PARAMETER :: NU_PHYSICAL=1.08 !cm^2/sec
	DOUBLE PRECISION,PARAMETER :: DIAM_PHYSICAL=0.4 !cm
	DOUBLE PRECISION,PARAMETER :: L_C=50.0	!Lattice Units Characteristic Length of the problem
	!DOUBLE PRECISION,PARAMETER :: U_T=RE_T*NU/L_C   ! Characteristic Velocity of Problem
	!DOUBLE PRECISION,PARAMETER :: A_G=-RE_S*NU*NU/((L_C**3)*(RHO_S-RHO_F)*RHO_F)
	
	DOUBLE PRECISION,PARAMETER :: A_C=0.5*L_C !Amplitude in LU
	DOUBLE PRECISION,PARAMETER :: FREQ_OSC=0.000274348 !0.05/(2.0*PI*A_C) !Angular Frequency of Oscillation=Max Vel LB/(2*pi*amplitude)
	DOUBLE PRECISION,PARAMETER :: U_C=RE_T*NU/L_C

	DOUBLE PRECISION,PARAMETER :: T_C=5.0*L_C/U_C
	DOUBLE PRECISION,PARAMETER :: F_C=0.5*U_C*U_C*L_C
	...
	DOUBLE PRECISION,PARAMETER :: A_G=0 !-9.81/C_G !Lattice Units
	DOUBLE PRECISION,PARAMETER :: Tau_y=Y_G*(RHO_S-RHO_F)*(-A_G)*L_C !BN_T*NU*U_T/L_C !yield stress

	!***Fluid Reological Properties
	DOUBLE PRECISION,PARAMETER :: Mu_0=NU !viscosity
	DOUBLE PRECISION,PARAMETER :: Mu_R=250*Mu_0 !unyeilded viscosity FOR BI-VISCOSITY MODEL
	DOUBLE PRECISION,PARAMETER :: M_PAP=1.0E5 !FOR PAPANASTASIOU MODEL 
    ...
    DOUBLE PRECISION,PARAMETER :: REY=1.0 !REYNOLDS NUMBER
	...
	TYPE WALL_PARTICLE
	DOUBLE PRECISION :: CENTER(1:2)
	DOUBLE PRECISION :: EQ_CENTER(1:2)
	DOUBLE PRECISION :: VELOCITY(1:3)
	DOUBLE PRECISION :: HEIGHT
	DOUBLE PRECISION :: WIDTH
	DOUBLE PRECISION :: THICKNESS
	DOUBLE PRECISION :: EDGE(1:4)
	DOUBLE PRECISION :: FORCE(1:3)
	DOUBLE PRECISION :: F_COL(1:2)
	DOUBLE PRECISION :: PHI(0:NX,0:NY)
	DOUBLE PRECISION :: ZITA !Interface Thickness
...
PROGRAM MAIN
    USE PARAM
    IMPLICIT NONE
    DOUBLE PRECISION F(0:Q-1,0:NX,0:NY),FEQ(0:Q-1,0:NX,0:NY),F_POST(0:Q-1,0:NX,0:NY),G(1:2,0:NX,0:NY)
    ...
    DOUBLE PRECISION :: AvgDen,SSUM2,ER,START,FINISH,MA,VP_OLD(1:3),INIT_WALL_CENTER(1:2),F_COL(1:2),PHI_T(0:NX,0:NY),RE_P,E_T,F_H_IN(1:3)
    INTEGER :: KK,I,J,IP,IW,Domain_Image(0:NX,0:NY)

	TYPE(PARTICLE) :: DISK(1) !???????
	TYPE(WALL_PARTICLE) :: WALL
	!********************************************************************************
	...
	PRINT*,'TAU=',1.0/OMEGA,''//CHAR(10)//'AND MA=',MA,''//CHAR(10)//'TAU_Y=',TAU_Y,' '//CHAR(10)//&
				&'C_L=',C_L,'cm'//CHAR(10)//'C_T=',C_T,'sec'//CHAR(10)//'C_U=',C_U,'cm/sec'//CHAR(10)//&
				&'C_G=',C_G,'M^2/sec'//CHAR(10)//'U_C=',U_C,'LU'
    IF(1.0/OMEGA < 0.5) THEN !CHECKING STABILITY CRITERIA FOR RELAXTION FREQUENCY
        ...
        STOP
    END IF
	!PRINT *,'If Simulation Parameters are OK continue'
	!PAUSE
	...
	OPEN(11,FILE='01_ParticleTimehistory.plt')
	WRITE(11,*)'VARIABLES=Itr,F_D,F_L'
	!WRITE(11,*)'VARIABLES=T,Y1,Y2,X1,X2,Ux1,Ux2,Uy1,Uy2,W1,W2,Fx1,Fx2,Fy1,Fy2,T1,T2,Dr,DX,DY'
	!WRITE(11,*)'VARIABLES=T,Y,Uy,Re_P,E_t'
	!WRITE(11,*)'VARIABLES=T,Y1,X1,Ux1,Uy1,W1,Fx1,Fy1,T1,Y_W,UY_W'
	...
	OPEN(57,FILE='0_P1_output.txt');
	OPEN(58,FILE='0_P2_output.txt');
	...
	END DO
	DO IP=1,N_PARTICLE
	READ(56+IP,*) DISK(IP)%CENTER,DISK(IP)%VELOCITY,DISK(IP)%FORCE
	END DO
	READ(59,*) OMEGA_MRT
	END IF
	...
	!PRINT *,'Particle Starts'
	!***WALL PARTICLES*****************************************************************************************
	WALL%CENTER=(/0.5*DBLE(NX),0.5*DBLE(NY)/);
	WALL%EQ_CENTER=(/0.5*DBLE(NX),0.5*DBLE(NY)/);
	WALL%HEIGHT=10.0*L_C; 
	WALL%WIDTH=15.0*L_C
	WALL%THICKNESS=10.0
	WALL%EDGE(1)=WALL%CENTER(2)-0.5*WALL%HEIGHT
	WALL%EDGE(2)=WALL%CENTER(1)+0.5*WALL%WIDTH
	WALL%EDGE(3)=WALL%CENTER(2)+0.5*WALL%HEIGHT
	WALL%EDGE(4)=WALL%CENTER(1)-0.5*WALL%WIDTH
	WALL%VELOCITY=(/DBLE(0.0),DBLE(0.0),DBLE(0.0)/) !initial velocity of particle 2.0*PI*A_C*COS(2.0*PI*FREQ_OSC*0.0)
	WALL%FORCE=(/0.0,0.0,0.0/) !initial forces on particle
	WALL%ZITA=1.0
	!***'Particles"**************************************************************
	DISK(1)%CENTER=(/0.5*DBLE(NX),0.5*DBLE(Ny)/) !x_p=x_l-left wall thickness;y_p=y_l-bottom wall thickness
	!DISK(2)%CENTER=(/(1.0/C_L),8.55/C_L/)
	DISK(1)%VELOCITY=(/-U_C,0.d0,0.d0/) !initial velocity of particle
	...
	CALL ComputeConcenterationFunction(DISK,WALL,DOMAIN_IMAGE,IS_SOLID_NODE,IS_PARTICLE_NODE,IS_WALL_NODE)
    ...
    !***MAIN LOOP***************************************
	KK=0;ER=1.0
	!KK=67050;ER=4.618714517246041E-002
    
    !DO WHILE(ER>1.0E-7)INT(2.0/C_T)
    DO WHILE(KK<INT(6.0*T_C))
	!PRINT *,KK
		KK=KK+1; !this loop computes variables for time kk+1 (velocities,...)

		!***Particle Dynamics Calculation****************
		IP=1
		VP_OLD=DISK(IP)%VELOCITY
		!DISK(IP)%VELOCITY(1)=DISK(IP)%VELOCITY(1)+DISK(IP)%FORCE(1)/DISK(IP)%M_P+F_H_IN(1)+DISK(IP)%F_COL(1)/DISK(IP)%M_P
		!DISK(IP)%VELOCITY(2)=DISK(IP)%VELOCITY(2)+DISK(IP)%FORCE(2)/DISK(IP)%M_P+F_H_IN(2)+(1.0-RHO0/DISK(IP)%RHO_P)*A_G+DISK(IP)%F_COL(2)/DISK(IP)%M_P
		!DISK(IP)%VELOCITY(3)=DISK(IP)%VELOCITY(3)+DISK(IP)%FORCE(3)/DISK(IP)%I_P+F_H_IN(3)
		DISK(IP)%VELOCITY(1)=-U_C*COS(2*PI*KK/T_C);DISK(IP)%VELOCITY(2)=0.0;DISK(IP)%VELOCITY(3)=0.0
		DISK(IP)%CENTER(1)=DISK(IP)%CENTER(1)+0.5*( DISK(IP)%VELOCITY(1)+VP_OLD(1) )
		DISK(IP)%CENTER(2)=DISK(IP)%CENTER(2)+0.5*( DISK(IP)%VELOCITY(2)+VP_OLD(2) )

		!***Wall Dynamics Calculation****************
		!WALL%VELOCITY(2)=2.0*PI*A_C*FREQ_OSC*SIN(2.0*PI*FREQ_OSC*KK)
		!WALL%CENTER(2)=WALL%EQ_CENTER(2)+A_C*COS(2.0*PI*FREQ_OSC*KK-PI)
		!WALL%VELOCITY(2)=2.0*PI*A_C*FREQ_OSC*SIN(2.0*PI*FREQ_OSC*KK+PI)
		!WALL%CENTER(2)=WALL%CENTER(2)+WALL%VELOCITY(2)
		!WALL%CENTER(2)=WALL%CENTER(2)+0.5*(WALL%VELOCITY(2)+2.0*PI*A_C*FREQ_OSC*SIN(2.0*PI*FREQ_OSC*(KK-1)+PI))
		!WALL%EDGE(1)=WALL%CENTER(2)-0.5*WALL%HEIGHT
		!WALL%EDGE(2)=WALL%CENTER(1)+0.5*WALL%WIDTH
		!WALL%EDGE(3)=WALL%CENTER(2)+0.5*WALL%HEIGHT
		!WALL%EDGE(4)=WALL%CENTER(1)-0.5*WALL%WIDTH
		...
		CALL Collision_MRT(UX,UY,RHO,F,FEQ,G,Omega_BN,DOMAIN_IMAGE)
		...
		CALL ComputeCollisionForce(DISK,WALL)

		!***Computing Internal Hydrodynamics force
		F_H_IN(1)=(RHO0/DISK(IP)%RHO_P)*DISK(IP)%M_P*(DISK(IP)%VELOCITY(1)-VP_OLD(1))
		F_H_IN(2)=(RHO0/DISK(IP)%RHO_P)*DISK(IP)%M_P*(DISK(IP)%VELOCITY(2)-VP_OLD(2))
		F_H_IN(3)=(RHO0/DISK(IP)%RHO_P)*DISK(IP)%I_P*(DISK(IP)%VELOCITY(3)-VP_OLD(3))

		!***Writing Results for Particle Dynamic and Kinematic		
		!RE_P=DISK(1)%RHO_P*DISK(1)%DIAM*SQRT(DISK(1)%VELOCITY(1)*DISK(1)%VELOCITY(1)+DISK(1)%VELOCITY(2)*DISK(1)%VELOCITY(2))/NU
		!E_T=0.5*DISK(1)%M_P*(DISK(1)%VELOCITY(1)*DISK(1)%VELOCITY(1)+DISK(1)%VELOCITY(2)*DISK(1)%VELOCITY(2))
		!WRITE(11,*) KK*C_T,(DISK(1)%CENTER(2))*C_L,DISK(1)%VELOCITY(2)*C_U,RE_P,E_T
		!PRINT *,KK,KK*C_T,(DISK(1)%CENTER(2))*C_L       
		
		!WRITE(11,*) KK*C_T,DISK(1)%CENTER(2)*C_L,DISK(1)%CENTER(1)*C_L,&
		!			&DISK(1)%VELOCITY(1)*C_U,DISK(1)%VELOCITY(2)*C_U,DISK(1)%VELOCITY(3),&
		!			&DISK(1)%FORCE(1),DISK(1)%FORCE(2),DISK(1)%FORCE(3),&
		!			&WALL%CENTER(2)*C_L,WALL%VELOCITY(2)*C_U
		!PRINT *,KK,(DISK(1)%CENTER(2))*C_L,WALL%EDGE(1)*C_L

		WRITE(11,*),KK/T_C,(DISK(1)%FORCE(1)+F_H_IN(1))/F_C,(DISK(1)%FORCE(1))/F_C
        PRINT*, KK,KK/T_C,(DISK(1)%FORCE(1)+F_H_IN(1))/F_C
		...
		IF(MOD(KK,1000)==0)THEN
			!IF(MOD(KK,2000)==0)THEN	
			!	DO J=0,NY
			!	DO I=0,NX
			!	IF(IS_SOLID_NODE(I,J))THEN
			!	DOMAIN_IMAGE(I,J)=10
			!	END IF
			!	END DO
			!	END DO		
				!CALL WriteResults(UX,UY,RHO,DOMAIN_IMAGE,kk)
			!	CALL WRITE_DOMAINIMAGE(DOMAIN_IMAGE,KK)
			!END IF
		...
		IF(KK==22500 .OR. KK==22916 .OR. KK==22917 .OR. KK==24583 .OR. KK==24584)THEN
			CALL WRITE_RESULTSPROFILE(UX,UY,RHO,KK)
		END IF

		!IF(MOD(KK,1063)==0)THEN
			!DO J=0,NY
			!DO I=0,NX
			!IF(IS_SOLID_NODE(I,J))THEN
			!DOMAIN_IMAGE(I,J)=10
	...
	END DO
	DO IP=1,N_PARTICLE
	WRITE(56+IP,*) DISK(IP)%CENTER,DISK(IP)%VELOCITY,DISK(IP)%FORCE
	END DO
	WRITE(59,*) OMEGA_MRT
...
!===================================================================
SUBROUTINE ComputeConcenterationFunction(DISK,WALL,DOMAIN_IMAGE,IS_SOLID_NODE,IS_PARTICLE_NODE,IS_WALL_NODE)
	USE PARAM,ONLY:NX,NY,DX,PARTICLE,PI,N_PARTICLE,WALL_PARTICLE,N_WALL
	IMPLICIT NONE
	DOUBLEPRECISION :: X_PC,Y_PC,R_P,D_I,EDGE_TOP,EDGE_BOTTOM,EDGE_RIGHT,EDGE_LEFT,D_X,D_Y
	INTEGER :: I,J,II,JJ,IP,IW,DOMAIN_IMAGE(0:NX,0:NY)
	TYPE(PARTICLE) :: DISK(1:N_PARTICLE)
	TYPE(WALL_PARTICLE) :: WALL 
	LOGICAL :: IS_SOLID_NODE(0:NX,0:NY),IS_WALL_NODE(0:NX,0:NY),IS_PARTICLE_NODE(0:NX,0:NY)

	DOMAIN_IMAGE=7;IS_SOLID_NODE=.FALSE.;IS_WALL_NODE=.TRUE.;IS_PARTICLE_NODE=.FALSE.
	WALL%PHI=1.0
	
	!***Wall Particle
	IF(0)THEN
	!**Compute Wall Edges
	EDGE_BOTTOM=WALL%EDGE(1)-WALL%THICKNESS
	EDGE_RIGHT=WALL%EDGE(2)+WALL%THICKNESS
	EDGE_TOP=WALL%EDGE(3)+WALL%THICKNESS
	EDGE_LEFT=WALL%EDGE(4)-WALL%THICKNESS
	DO J=0,NY
		DO I=0,NX
			IF(DBLE(I)<=WALL%EDGE(2) .AND. DBLE(I)>=WALL%EDGE(4) .AND. DBLE(J)>=WALL%EDGE(1) .AND. DBLE(J)<=WALL%EDGE(3))THEN
				D_I=MIN(ABS(DBLE(J)-WALL%EDGE(3)),ABS(DBLE(J)-WALL%EDGE(1)),ABS(DBLE(I)-WALL%EDGE(4)),ABS(DBLE(I)-WALL%EDGE(2)))
				IF( D_I<=-0.5*WALL%ZITA )THEN
					WALL%PHI(I,J)=0.0
				ELSEIF( ABS(D_I)<0.5*WALL%ZITA )THEN
					WALL%PHI(I,J)=0.5*( SIN( PI*D_I/WALL%ZITA )+1.0 )
				ELSEIF( D_I>=0.5*WALL%ZITA )THEN
					WALL%PHI(I,J)=1.0
				END IF
				WALL%PHI(I,J)=-(WALL%PHI(I,J)-1.0)
				IS_WALL_NODE(I,J)=.FALSE.
				DOMAIN_IMAGE(I,J)=0
			ELSEIF(DBLE(I)<=EDGE_LEFT .OR. DBLE(I)>=EDGE_RIGHT .OR. DBLE(J)<=EDGE_BOTTOM .OR. DBLE(J)>=EDGE_TOP)THEN
				D_X=MAX(ABS(DBLE(I)-WALL%CENTER(1))-0.5*WALL%WIDTH-WALL%THICKNESS,DBLE(0.0));
				D_Y=MAX(ABS(DBLE(J)-WALL%CENTER(2))-0.5*WALL%HEIGHT-WALL%THICKNESS,DBLE(0.0));
				D_I=-(D_X**2+D_Y**2);
				IF( D_I<=-0.5*WALL%ZITA )THEN
					WALL%PHI(I,J)=0.0
				ELSEIF( ABS(D_I)<0.5*WALL%ZITA )THEN
...
!===================================================================
SUBROUTINE ComputeForceField_SPM(G,DISK,WALL,UX,UY)
	USE PARAM,ONLY:NX,NY,DT,PARTICLE,N_PARTICLE,WALL_PARTICLE,N_WALL
	IMPLICIT NONE
...
!
!===================================================================
SUBROUTINE ComputeCollisionForce(DISK,WALL)  
	USE PARAM
	IMPLICIT NONE
	TYPE(PARTICLE) :: DISK(1:N_PARTICLE)
	TYPE(WALL_PARTICLE) :: WALL
	...
	DO IP=1,N_PARTICLE
		DO IW=1,4 !Index of each Wall
			...
			!R_W(1)=DISK(IP)%CENTER(1);R_W(2)=2.0*(WALL(IW)%CENTER(2)+0.5*WALL(IW)%HEIGHT)-DISK(IP)%CENTER(2) !  !Glowinsky
			R_W(1)=DISK(IP)%CENTER(1);R_W(2)=2.0*WALL%EDGE(IW)-DISK(IP)%CENTER(2)
			!R_W(1)=DISK(IP)%CENTER(1);R_W(2)=WALL(IW)%HEIGHT-0.5*DISK(IP)%DIAM
			ELSEIF(IW==2)THEN !RIGHT WALL
			R_W(1)=2.0*WALL%EDGE(IW)-DISK(IP)%CENTER(1);R_W(2)=DISK(IP)%CENTER(2)  !2.0*DBLE(NX)-DISK(IP)%CENTER(1)
...
!==========================================================================================
SUBROUTINE Collision_MRT(UX,UY,RHO,F,FEQ,G,Omega_BN,Domain_Image)
    use param,only : nx,ny,Omega_mrt,MU_0,M_PAP,TAU_Y,CX,CY,mu_r,gamma_c,NON_NEWTONIAN
    IMPLICIT NONE
    DOUBLE PRECISION,intent(in) :: rho(0:nx,0:ny),ux(0:nx,0:ny),uy(0:nx,0:ny)
    DOUBLE PRECISION,intent(inout) :: f(0:8,0:nx,0:ny),feq(0:8,0:nx,0:ny),G(1:2,0:NX,0:NY),Omega_BN(0:NX,0:NY)
	INTEGER,INTENT(INOUT) :: Domain_Image(0:NX,0:NY)
    DOUBLE PRECISION :: M(0:8),MF(0:8),MEQ(0:8),M_NE(0:8),F_1(0:8),ZARIB,D11,D22,D12,GAMMA,NU_A,SUM,M1SM(0:8,0:8),TAU_11,TAU_22,TAU_12,TAU_MAG,d_mrt=1./36.
    INTEGER :: i,j,K,II,JJ
    ...
    DO J=0,nY
        DO I=0,nX
			...
			IF(NON_NEWTONIAN)THEN
			CALL COMPUTEFEQNEW(UX(I,J),UY(I,J),RHO(I,J),FEQ(:,I,J))
			MEQ(0)=FEQ(0,I,J)+FEQ(1,I,J)+FEQ(2,I,J)+FEQ(3,I,J)+FEQ(4,I,J)+FEQ(5,I,J)+FEQ(6,I,J)+FEQ(7,I,J)+FEQ(8,I,J)
			MEQ(1)=-4.0*FEQ(0,I,J)-FEQ(1,I,J)-FEQ(2,I,J)-FEQ(3,I,J)-FEQ(4,I,J)+2.0*(FEQ(5,I,J)+FEQ(6,I,J)+FEQ(7,I,J)+FEQ(8,I,J))
			MEQ(2)=4.0*FEQ(0,I,J)-2.0*(FEQ(1,I,J)+FEQ(2,I,J)+FEQ(3,I,J)+FEQ(4,I,J))+(FEQ(5,I,J)+FEQ(6,I,J)+FEQ(7,I,J)+FEQ(8,I,J))
			MEQ(3)=FEQ(1,I,J)-FEQ(3,I,J)+FEQ(5,I,J)-FEQ(6,I,J)-FEQ(7,I,J)+FEQ(8,I,J)
			MEQ(4)=-2.0*(FEQ(1,I,J)-FEQ(3,I,J))+FEQ(5,I,J)-FEQ(6,I,J)-FEQ(7,I,J)+FEQ(8,I,J)
			MEQ(5)=FEQ(2,I,J)-FEQ(4,I,J)+FEQ(5,I,J)+FEQ(6,I,J)-FEQ(7,I,J)-FEQ(8,I,J)
			MEQ(6)=-2.0*(FEQ(2,I,J)-FEQ(4,I,J))+FEQ(5,I,J)+FEQ(6,I,J)-FEQ(7,I,J)-FEQ(8,I,J)
			MEQ(7)=FEQ(1,I,J)-FEQ(2,I,J)+FEQ(3,I,J)-FEQ(4,I,J)
			MEQ(8)=FEQ(5,I,J)-FEQ(6,I,J)+FEQ(7,I,J)-FEQ(8,I,J)
			M_NE(:)=(M(:)-MEQ(:));
			END IF
			...
			IF(NON_NEWTONIAN)THEN
			!***YU Method for Strain Rate Tensor
			!M_NE(:)=(M(:)-MEQ(:))/Rho(I,J);
			!M_NE(:)=M_NE(:)/Rho(I,J)
			!D11=-0.25*OMEGA_MRT(1)*M_NE(1)-0.75*OMEGA_MRT(7)*M_NE(7)
			!D22=-0.25*OMEGA_MRT(1)*M_NE(1)+0.75*OMEGA_MRT(7)*M_NE(7)
			!D12=-1.5*OMEGA_MRT(7)*M_NE(8)
			...
			ZARIB=-1.5*d_mrt/RHO(I,J)
			D11=ZARIB*(F_1(1)+F_1(5)+F_1(6)+F_1(3)+F_1(7)+F_1(8)) !rate of deformation tensor Components d_ij
			D22=ZARIB*(F_1(5)+F_1(2)+F_1(6)+F_1(7)+F_1(4)+F_1(8))
			D12=ZARIB*(F_1(5)-F_1(6)+F_1(7)-F_1(8))
			!***SRT Method
			!M_NE(:)=(M(:)-MEQ(:));
			!F_1(0)=d_mrt*( 4.0*(m_NE(0)-m_NE(1)+m_NE(2)) )
			!F_1(1)=d_mrt*( 4.*m_NE(0)-m_NE(1)-2.*m_NE(2)+6.*m_NE(3)-6.*m_NE(4)+9.*m_NE(7) )
			!F_1(2)=d_mrt*( 4.0*M_NE(0)-M_NE(1)-2.0*M_NE(2)+6.0*M_NE(5)-6.0*M_NE(6)-9.0*M_NE(7) )
			!F_1(3)=d_mrt*( 4.0*M_NE(0)-M_NE(1)-2.0*M_NE(2)-6.0*M_NE(3)+6.0*M_NE(4)+9.0*M_NE(7) )
			!F_1(4)=d_mrt*( 4.0*M_NE(0)-M_NE(1)-2.0*M_NE(2)-6.0*M_NE(5)+6.0*M_NE(6)-9.0*M_NE(7) )
			!F_1(5)=d_mrt*( 4.0*M_NE(0)+2.0*M_NE(1)+M_NE(2)+6.0*M_NE(3)+3.0*M_NE(4)+6.0*M_NE(5)+3.0*M_NE(6)+9.0*M_NE(8) )
			!F_1(6)=d_mrt*( 4.0*M_NE(0)+2.0*M_NE(1)+M_NE(2)-6.0*M_NE(3)-3.0*M_NE(4)+6.0*M_NE(5)+3.0*M_NE(6)-9.0*M_NE(8) )
			!F_1(7)=d_mrt*( 4.0*M_NE(0)+2.0*M_NE(1)+M_NE(2)-6.0*M_NE(3)-3.0*M_NE(4)-6.0*M_NE(5)-3.0*M_NE(6)+9.0*M_NE(8) )
			!F_1(8)=d_mrt*( 4.0*M_NE(0)+2.0*M_NE(1)+M_NE(2)+6.0*M_NE(3)+3.0*M_NE(4)-6.0*M_NE(5)-3.0*M_NE(6)-9.0*M_NE(8) )
			!CALL COMPUTEFEQNEW(UX(I,J),UY(I,J),RHO(I,J),FEQ(:,I,J))
			!F_1(:)=(F(:,I,J)-FEQ(:,I,J))
			!ZARIB=-1.5*OMEGA_BN(I,J)/RHO(I,J)
			!D11=ZARIB*(F_1(1)+F_1(5)+F_1(6)+F_1(3)+F_1(7)+F_1(8)) !rate of deformation tensor Components d_ij
			...
			!NU_A=NU_A/RHO(I,J)
			!***Papanastasiou Model***********************************
			IF(GAMMA<1.0E-15)THEN !zero shear rate limit 
				Nu_A=Mu_0+M_PAP*TAU_Y !un-yielded Viscosity
			ELSE
				Nu_A=Mu_0+(TAU_Y/GAMMA)*(1.0-EXP(-M_PAP*GAMMA)) 
			END IF
...
!===================================================================
SUBROUTINE BOUNDARYCONDITION(F,F_POST,RHO,UX,UY)
    USE PARAM
    IMPLICIT NONE
	...
	UXN=0.0;UYN=0.0
	UXS=0.0;UYS=0.0
	!UN=-UX_IN !Velocity of Top Wall
	!US=UX_IN  !Velocity of Bottom Wall
	DO I=1,NX-1
	...
	IF(0)THEN
	I=0;J=0;
	Ux_w=0.0;Uy_w=0.0;Rho_w=Rho(I,J);
...
!===================================================================
SUBROUTINE WriteResults(UX,UY,RHO,DOMAIN_IMAGE,TSTEP)
    USE PARAM
    IMPLICIT NONE
    DOUBLE PRECISION,INTENT(IN) :: RHO(0:NX,0:NY),UX(0:NX,0:NY),UY(0:NX,0:NY)
	...
	OPEN(10,FILE='02_VelocityField_'//TRIM(FILENAME)//'.plt')

	LSCALE=1.0 !(2.0/L_C)
	USCALE=1.0 !C_U !(1.0/U_C)
    
    WRITE(10,*)'VARIABLES =X, Y, UX, UY, RHO,Solid_Nodes'
    WRITE(10,*)'ZONE ','I=',NX+1,'J=',NY+1,',','F=BLOCK'
    DO J=0,NY
        WRITE(10,*)(DBLE(I)*LSCALE,I=0,NX)
    END DO
    DO J=0,NY
        WRITE(10,*)(DBLE(J)*LSCALE,I=0,NX)
    END DO
    DO J=0,NY
        WRITE(10,*)(UX(I,J)*USCALE,I=0,NX)
    END DO
    DO J=0,NY
        WRITE(10,*)(UY(I,J)*USCALE,I=0,NX)
    END DO
    DO J=0,NY
        WRITE(10,*)(RHO(I,J),I=0,NX)
    END DO
	DO J=0,NY
        WRITE(10,*)(DBLE(DOMAIN_IMAGE(I,J)),I=0,NX)
    END DO
...
SUBROUTINE WRITE_RESULTSPROFILE(UX,UY,RHO,TSTEP)
    ...
    WRITE(15,*)'VARIABLES = y/D,ux_6d,ux_0,ux_p6d,ux_12d,uy_6d,uy_0,uy_p6d,uy_12d'
    DO J=0,NY
        WRITE(15,*) (DBLE(J)-DBLE(NY/2))*lscale,UX(NX/2-30,J)*USCALE,UX(NX/2,J)*USCALE,UX(NX/2+30,J)*USCALE,UX(NX/2+60,J)*USCALE,&
					&UY(NX/2-30,J)*USCALE,UY(NX/2,J)*USCALE,UY(NX/2+30,J)*USCALE,UY(NX/2+60,J)*USCALE
    END DO
    CLOSE(15)
...
SUBROUTINE RESULTSPROFILE(UX,UY,RHO)
    ...
    DO J=0,NY
        ...
        WRITE(10,*) DBLE(J),UX(NX/2,J),UX_R
    END DO
    CLOSE(10)
end SUBROUTINE
!====================================================================================
